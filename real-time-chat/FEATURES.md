...
### 4. Smart UI Features & System Messages
...
* **Join/Leave Notifications:** The Go Hub detects connection events (`h.register` and `h.unregister`) and broadcasts system-generated alerts (e.g. `"Alice joined the chat"`, `"Bob left the chat"`) directly to clients.
* **Visual Styling Separation:** System events are styled differently on the frontend (centered, italicized) compared to regular chat bubbles.
...
* Clean Go tear-down: The Go hub automatically deregisters clients when they close their browser tab, terminating their respective reading and writing Goroutines and triggering leave notifications.
